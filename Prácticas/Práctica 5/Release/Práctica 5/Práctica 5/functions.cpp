...
int menu(tListaEdificios &edificios, tListaJugadores &jugadores, int pos) {
	...
	case 1: 
		for(int i = 0; i < jugadores.jugador[pos].comprados.cont; i++) {
			if(buscarEdificio(edificios, 0, edificios.cont - 1, jugadores.jugador[pos].comprados.elem[i], pos_edificio))
...
int menu_admin(tListaJugadores &jugadores, tListaEdificios &edificios) {
	...
	case 1: 
		...
		break;
	case 2: 
		if(listaEdificiosLlena(edificios))
			ampliar_edificios(edificios);
	...
	case 4: 
		...
		break;
	case 5: 
		if(listaJugadoresLlena(jugadores)) 
			ampliar_jugadores(jugadores);

		if(insertarJugador(jugadores, valido)) {
			if(valido)
...
void ejecutar_turno(tListaJugadores &jugadores, tListaEdificios edificios) {
	...
	for(int i = 0; i < jugadores.cont; i++) {
		for(int j = 0; j < jugadores.jugador[i].comprados.cont; j++) {
			buscarEdificio(edificios, 0, edificios.cont - 1, jugadores.jugador[i].comprados.elem[j], pos);
